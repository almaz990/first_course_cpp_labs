...
class WorkerWithHourlyPay : public WorkerWithFixedSalary
{
private:
    int hours;
public:
    WorkerWithHourlyPay() : WorkerWithFixedSalary()
    {
    ...
    double tax();
    double cash();
    void operator<<(WorkerWithHourlyPay f)
    {
        std::cout << getSurname() << " " << getName()[0] << " " << Age() << " " << getGender()
            << "\n" << wage() << 'р' << ' ' << tax() << "р " << cash() << "р\n";
    }
...
double WorkerWithHourlyPay::wage()
{
    return hours * salary;
}

double WorkerWithHourlyPay::tax()
{
    return wage() * (13.0 / 100.0);
}

double WorkerWithHourlyPay::cash()
{
    return wage() - tax();
}
